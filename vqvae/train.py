...
def train(model, train_dataloader, val_dataloader, opt):
    ...
    for epoch in range(opt.num_epochs):
        ...
        for step, (inputs, filename) in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch}/{opt.num_epochs}", total=len(train_dataloader))):
            inputs = inputs.to("cuda")
            #vqvae.set_input(inputs)
            quant, loss, info = vqvae.vqvae_module.encode(inputs)
            reconstructions = vqvae.vqvae_module.decode(quant)
            vqvae.x_recon=reconstructions
            vqvae.x=inputs
            vqvae.qloss=loss
            vqvae.optimizer.zero_grad(set_to_none=True)
            vqvae.backward()
            vqvae.optimizer.step()
            #output, loss = vqvae.vqvae(inputs)
            model.optimize_parameters(total_steps)

            # 记录损失值到 TensorBoard
            writer.add_scalar('Loss/total_loss', loss.item(), epoch * total_steps + step)

            out_img = sitk.GetImageFromArray(reconstructions[0:1].cpu().detach())
            ...
            if step % opt.print_freq == 0:
                ...
                print(f"[Epoch {epoch}/{opt.num_epochs}] [Step {step}/{total_steps}] Losses: {errors}")

        # 在每个 epoch 结束时保存检查点
        if epoch % opt.save_epoch_freq == 0:
            model.save(os.path.join(ckpt_dir, f'epoch_{epoch}.pth'), global_step=epoch, save_opt=True)

        # 记录训练过程中的平均 SSIM 和 MSE 到 TensorBoard
        avg_ssim = total_ssim / len(train_dataloader)
        avg_mse = total_mse / len(train_dataloader)
        writer.add_scalar('Metrics/Average_ssim', avg_ssim, epoch)
        writer.add_scalar('Metrics/Average_mse', avg_mse, epoch)

        print(f'Training : Average_ssim: {avg_ssim} , Average_mse: {avg_mse}')
        ...
        with torch.no_grad():
            ...
            avg_ssim_val = total_ssim / len(val_dataloader)
            avg_mse_val = total_mse / len(val_dataloader)
