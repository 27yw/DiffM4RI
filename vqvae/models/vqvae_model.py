...
class VQVAEModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            # self.optimizer = optim.Adam(self.vqvae.parameters(), lr=4.5e-4, betas=(0.5, 0.9))
    ...
    def forward(self):
        ...
        return self.x_recon, self.qloss
    ...
    def inference(self, data, should_render=False, verbose=False):
        ...
        with torch.no_grad():
            self.z = self.vqvae(data, forward_no_quant=True, encode_only=True)
            self.x_recon = self.vqvae_module.decode_no_quant(self.z)

        self.switch_train()
        return self.z,self.x_recon
    ...
    def save(self, filename, global_step=0, save_opt=False):
