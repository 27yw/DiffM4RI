...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        """
        # tar_shape = x_start.shape
        if noise is None:
            #print("x_start.shape",x_start.shape)
            noise = th.randn_like(x_start)
        #assert noise.shape == x_start.shape
        # mean = var_to_tensor(self.sqrt_alphas_cumprod, t.cpu(), tar_shape, "cuda")
        # std = var_to_tensor(self.sqrt_one_minus_alphas_cumprod, t.cpu(), tar_shape, "cuda")
        # print("x_start * mean",x_start * mean.shape)
        # print("noise * std", noise * std.shape)
        # return x_start * mean + noise * std
        return (
    ...
    def inpaint_sample_loop(
        ...
        final = None
        final_list = []
        for sample in self.inpaint_sample_loop_progressive(
            model,
            shape,
            noise=noise,
            clip_denoised=clip_denoised,
            ...
            final = sample
            final_list.append(final["sample"])
        if return_all:
            return final,final_list
        else:
            return final["sample"],final_list

    def inpaint_sample_loop_progressive(
        self,
        model,
        shape,
        noise=None,
    ...
    def training_losses(self, model, x_start1, t, model_kwargs=None, noise=None):
        """
        ...
        # print("=========================after")
        # print(torch.max(x_start))
        # print(torch.min(x_start))
        ...
        if noise is None:
            ...
            #     None, x_start.shape[-3:],
            #     num_timesteps=self.num_timesteps,
            #     num_batches=x_start.size(0),
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                # print(model_output.shape)
                # print(len(th.split(model_output, C, dim=1)))
                model_output, model_var_values = th.split(model_output, C, dim=1)
            ...
            #print("print(model_output.shape)",model_output.shape)
            assert model_output.shape == target.shape == x_start.shape
            terms["mse"] = mean_flat((target - model_output) ** 2)
            reshaped_scaled_data = model_output.reshape(x_start1.size(0), -1)
            # 使用逆转min-max公式
            unscaled_data = reshaped_scaled_data * (max_vals - min_vals) + min_vals
            # 将逆转归一化后的数据重新reshape为3D形状
            unscaled_data_3d = unscaled_data.reshape(x_start1.shape)
            if "vb" in terms:
